...
unsigned int main()
{
    GetMd5SumFromStr("abc");

    //ifstream file;
    ////file.open("C:\\Users\\yangy9.CCR\\Desktop\\model and kernel\\media_model.bin", ios::in | ios::binary);
    //file.open("D:\\md5\\test.txt", ios::in);
    //istreambuf_iterator<char> begin(file), eof;
    //string str(begin, eof);
    //auto start = std::chrono::system_clock::now();
    //string s = getMD5(str);
    //auto end = std::chrono::system_clock::now();
    //std::chrono::duration<double> diff = end - start;
    //std::cout << "\nTime span:" << diff.count() << " s\n";
    //cout << s;
    return 0;
}